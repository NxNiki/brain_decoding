...
def CMLReadDFRow(row):
    ...
    return CMLReader(
        rd["subject"], rd["experiment"], rd["session"], montage=int(rd["montage"]), localization=int(rd["localization"])
    )
...
def set_pubfig():
    ...
    sb.set_context(
        "paper",
        rc={"font.size": 14, "axes.titlesize": 14, "axes.labelsize": 14, "axes.ticksize": 12, "axes.titlesize": 14},
    )
...
def plotHistoBar(values, start, end, bin_size, tick_range_divisor=1, normalize=False, log_scale=False):
    ...
    bins = np.arange(
        start, end + bin_size + bin_size / 1000, bin_size
    )  # added bin+0.001 to show last bin and last tick
    if log_scale == True:
...
def tscorePlot(mdl, yrange=7.5, names=None):
    ...
    xr = np.linspace(-0.5, len(mdl.tvalues) - 0.5, 100)
...
def csvWriter(lists, filename):
    ...
    with open(str(filename) + ".csv", "w", newline="") as csvfile:
        csvwriter = csv.writer(csvfile, delimiter=",", quotechar="|", quoting=csv.QUOTE_MINIMAL)  # comma separate
        for t in lists:
