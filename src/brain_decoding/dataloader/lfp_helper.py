...
NCS_RECORD = np.dtype(
    [
        ("TimeStamp", np.uint64),  # Cheetah timestamp for this record. This corresponds to
...
def load_ncs(file_path, load_time=True, rescale_data=True, signal_scaling=MICROVOLT_SCALING):
    ...
    if load_time:
        ...
        times = np.interp(np.arange(num_samples), np.arange(0, num_samples, 512), records["TimeStamp"]).astype(
            np.uint64
        )
        ncs["time"] = times
        ncs["time_units"] = "µs"
